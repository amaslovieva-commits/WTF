...
class ExpenseTracker:
    def __init__(self, root):
        ...
        # Фильтры
        tk.Label(root, text="Фильтр категории:").grid(row=4, column=0)
        self.filter_cat = ttk.Combobox(root, values=["Все", "Еда", "Транспорт", "Развлечения", "Другое"])
        self.filter_cat.set("Все")
        self.filter_cat.grid(row=4, column=1)
        self.filter_cat.bind("<<ComboboxSelected>>", lambda e: self.update_table())
        ...
        # Итог
        self.total_label = tk.Label(root, text="Итого за период: 0", font=('Arial', 10, 'bold'))
    ...
    def update_table(self):
        ...
        for ex in self.expenses:
            if cat_filter == "Все" or ex['category'] == cat_filter:
                self.tree.insert("", tk.END, values=(ex['amount'], ex['category'], ex['date']))
                total += ex['amount']
        
        self.total_label.config(text=f"Итого за период: {total}")
